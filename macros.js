function onFormSubmit(e) {
  ...
  // 1) The start date cannot be in the past
  const now = new Date();
  if (startDate < now) {
    sendDenial(email, name, "The start date cannot be in the past.", bookingSummary);
    return;
  }
  // 2) The start date must be within three months
  ...
  // 3) The end date cannot be earlier than the start date
  if (endDate < startDate) {
    sendDenial(email, name, "The end date cannot be earlier than the start date.", bookingSummary);
    return;
  }
  // 4) The booking cannot be longer than four days
  ...
  // 5) Double bookings are not allowed
  ...
  for (let event of futureEvents) {
    ...
    if (desc.includes(userIdentifier)) {
      sendDenial(email, name, "You already have a future booking. Only one booking is allowed at a time.", bookingSummary);
      return;
    }
  }
  // 6) The dates must be available
